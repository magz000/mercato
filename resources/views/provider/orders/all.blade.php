...
@section('content')
    ...
    <div class="container">
        ...
        <div class="row">
            ...
            <div class="col-md-10" id="orders-container">
                ...
                @foreach ($order_ids as $key => $order_id)
                    ...
                    <div class="panel panel-default" {!! $count_pending > 0 ? 'style="border: 7px solid #00f"' : ''  !!}>
                        <div class="panel-heading">
                            ...
                            <div class="pull-right small"><span
                                        class="label label-{{ $OrderModel->state($order->status)['state'] }}">{{ $OrderModel->state($order->status)['value'] }}</span>
                            </div>
                                    ...
                                    <tr>
                                        <td>
                                            <div class="avatar med"
                                                 style="background-image: url('{{ asset('img/uploads/' . $ProductModel->find($content->product_id)->picture) }}')"></div>
                                        </td>
                                        <td>
                                            {{ $ProductModel->find($content->product_id)->name }}<br/>
                                            Served By : <a
                                                    href="{{ route('userPage', $ProviderModel->find($content->provider_id)->username) }}">{{ $ProviderModel->find($content->provider_id)->firstname . ' ' . $ProviderModel->find($content->provider_id)->lastname }}</a>
                                        </td>
                                        ...
                                        <td>
                                            <span class="label label-{{ $OrderContentModel->state($content->status)['state'] }}">{{ $OrderContentModel->state($content->status)['value'] }}</span>
                                        </td>
                                        <td>
                                            <button type="button" data-toggle="tooltip" title="{{ $content->note }}"
                                                    class="btn btn-default btn-xs"><i class="fa fa-fw fa-eye"> </i>
                                            </button>
                                        </td>
                                        <td>
                                            @if ($content->status < 3)
                                                <button type="button" class="btn btn-default btn-xs __updateTrigger"
                                                        data-ocid="{{ $content->id }}"
                                                        data-status="{{ $content->status }}" data-toggle="modal"
                                                        data-target="#__updateOrder"><i class="fa fa-fw fa-pencil"> </i>
                                                </button>
                                            @else
                                                -
                                            @endif
                                                    ...
                                                    <div class="col-md-3">
                                                        <center>
                                                            <select class="__rating_selected">
                                                                <option value="1"
                                                                        {{ $rating->value == 1 ? 'selected' : '' }} data-html="Very Good">
                                                                    1
                                                                </option>
                                                                <option value="2"
                                                                        {{ $rating->value == 2 ? 'selected' : '' }} data-html="Good">
                                                                    2
                                                                </option>
                                                                <option value="3"
                                                                        {{ $rating->value == 3 ? 'selected' : '' }} data-html="Average">
                                                                    3
                                                                </option>
                                                                <option value="4"
                                                                        {{ $rating->value == 4 ? 'selected' : '' }} data-html="Poor">
                                                                    4
                                                                </option>
                                                                <option value="5"
                                                                        {{ $rating->value == 5 ? 'selected' : '' }} data-html="Very Poor">
                                                                    5
                                                                </option>
                                                            </select>
                                                        </center>
        ...
        <div class="modal-dialog modal-sm">
            ...
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <button type="button" id="__updateOrderBtn" class="btn btn-success" data-dismiss="modal">Update
                        Order
                    </button>
...
@section('scripts')
    <script type="text/javascript">
        $(function () {
            ...
            $('#__updateOrderBtn').click(function () {
                ...
                $.ajax({
                    ...
                    data: {
                        ...
                        status: $('select[name="content_status"]').val()
                    },
                    headers: {
            ...
            $('#orders-container').on('click', '.__updateTrigger', function () {
            ...
            //     var ocid = $(this).attr('data-ocid');
            //
            //     alert($(this).attr('data-status') + '-test-' + ocid);
            //
            //
            //     $('input[name="content_id"]').val(ocid);
            //     $('select[name="content_status"]').val($(this).attr('data-status'));
        ...
        setInterval(function () {
            $.get('{{route("providerOrderPageAjax")}}', {}, function (data) {
